let input = `LLL.LLL.LLLLL.LLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLL.LLLLLLLLL.LLLLLLLLLLLLLLLL.LLLLLL.LLLLLLLL
...
function firstVisible(seats, row, col, direction) {
  let firstVisible = null;
  for (
    let dist = 1;
    row + dist * direction[0] >= 0 &&
    col + dist * direction[1] >= 0 &&
    row + dist * direction[0] < seats.length &&
    col + dist * direction[1] < seats[row].length;
    dist++
  ) {
    firstVisible = seats[row + dist * direction[0]][col + dist * direction[1]];
    if (firstVisible == "L" || firstVisible == "#") break;
  }
  return firstVisible;
}
...
function next(seats) {
  ...
  for (let row = 0; row < seats.length; row++) {
    for (let col = 0; col < seats[row].length; col++) {
      /* rule #1 - occupy if no VISIBLE seats occupied in 8 directions */
      if (
        newSeats[row][col] == "L" &&
        directions.every((direction) => {
          let first = firstVisible(seats, row, col, direction);
          return (
            row + direction[0] < 0 ||
            col + direction[1] < 0 ||
            row + direction[0] >= seats.length ||
            col + direction[1] >= seats[row].length ||
            first != "#"
          );
        })
      ) {
        ...
        /* rule 2 - occupied seats empty if 5 or more visible occupied seats */
        newSeats[row][col] == "#" &&
